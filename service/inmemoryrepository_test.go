...
var (
	fakeDBURIIN = "mongodb://fake.uri@addr:port/guid"
)

func TestAddMatchShowsUpInMongoRepositoryIn(t *testing.T) {
...
func TestGetMatchRetrievesProperMatchFromMongoIn(t *testing.T) {
	fakes.TargetCount = 1
	var fakeMatches = []matchRecord{}
	var matchesCollection = cfmgo.Connect(
		fakes.FakeNewCollectionDialer(fakeMatches),
...
func TestGetNonExistentMatchReturnsErrorIn(t *testing.T) {
	fakes.TargetCount = 0
	var fakeMatches = []matchRecord{}
	var matchesCollection = cfmgo.Connect(
		fakes.FakeNewCollectionDialer(fakeMatches),
